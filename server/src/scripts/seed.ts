...
import { User, Project, Certificate, TimelineItem, Skill, ContactMessage } from '../models';
...
async function createAdminUser() {
  try {
    ...
    if (!env.ADMIN_EMAIL || !env.ADMIN_PASSWORD) {
      logger.warn('ADMIN_EMAIL or ADMIN_PASSWORD not provided in environment variables');
      return null;
    }

    const adminUser = new User({
      email: env.ADMIN_EMAIL,
      passwordHash: env.ADMIN_PASSWORD, // Will be hashed by the pre-save hook
      role: 'admin'
    });
...
async function seedProjects() {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Projects already exist (${existingCount} found), skipping...`);
      return;
    }

    await Project.insertMany(sampleProjects);
    logger.info(`✅ Created ${sampleProjects.length} projects`);
  } catch (error) {
...
async function seedCertificates() {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Certificates already exist (${existingCount} found), skipping...`);
      return;
    }

    await Certificate.insertMany(sampleCertificates);
    logger.info(`✅ Created ${sampleCertificates.length} certificates`);
  } catch (error) {
    logger.error('Error seeding certificates:', error);
    throw error;
  }
}
...
async function seedTimelineItems() {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Timeline items already exist (${existingCount} found), skipping...`);
      return;
    }

    await TimelineItem.insertMany(sampleTimelineItems);
    logger.info(`✅ Created ${sampleTimelineItems.length} timeline items`);
  } catch (error) {
    logger.error('Error seeding timeline items:', error);
    throw error;
  }
}
...
async function seedSkills() {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Skills already exist (${existingCount} found), skipping...`);
      return;
    }

    await Skill.insertMany(sampleSkills);
    logger.info(`✅ Created ${sampleSkills.length} skills`);
  } catch (error) {
...
async function seedContactMessages() {
  try {
    ...
    if (existingCount > 0) {
      logger.info(`Contact messages already exist (${existingCount} found), skipping...`);
      return;
    }

    await ContactMessage.insertMany(sampleContactMessages);
    logger.info(`✅ Created ${sampleContactMessages.length} contact messages`);
  } catch (error) {
    logger.error('Error seeding contact messages:', error);
    throw error;
  }
}
